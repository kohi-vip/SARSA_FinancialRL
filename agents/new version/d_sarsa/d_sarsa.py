...
class Qsa(nn.Module):
    def __init__(self, input_size=9, num_classes=11):  # 11 actions for k=5, 9D state with VWAP and CORR_FX
        super().__init__()
        # Simplified architecture without BatchNorm and Dropout
...
def train_deep_sarsa(mdp, train_series, test_series, verbose=False, episodes=30, gamma=0.6, alpha=0.7, epsilon_start=0.8, epsilon_min=0.2, epsilon_decay=0.9, nn_epochs=10, nn_lr=1e-5):
    ...
    qsa = Qsa(input_size=9, num_classes=len(mdp.A))
    
    # Initial state from first training sample
    first_row = train_series.iloc[0]
    # State: [price, balance, shares, MACD, RSI, CCI, ADX, VWAP, CORR_FX]
    state_init = [
        ...
        float(first_row['CCI']),
        float(first_row['ADX']),
        float(first_row['VWAP']),
        float(first_row['CORR_FX'])
    ]
