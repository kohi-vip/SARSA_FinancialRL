...
class StockTradingMDP:
    ...
    def get_features(self, s, new_record):
        ...
        Trả về: [MACD, RSI, CCI, ADX, VWAP, CORR_FX]
        """
        # Các cột: date(0), open(1), high(2), low(3), close(4), volume(5), symbol(6), MACD(7), RSI(8), CCI(9), ADX(10), VWAP(11), CORR_FX(12)
        return [
            float(new_record['MACD']),
            float(new_record['RSI']),
            float(new_record['CCI']),
            float(new_record['ADX']),
            float(new_record['VWAP']),
            float(new_record['CORR_FX'])
        ]
    ...
    def update_state(self, s, a, new_record):
        ...
        # update state: [price, balance, shares, MACD, RSI, CCI, ADX, VWAP, CORR_FX]
        # cập nhật trạng thái: [giá, số dư, số cổ phiếu, MACD, RSI, CCI, ADX, VWAP, CORR_FX]
    ...
    def interact_test(self, pi, train_series, test_series, series_name='test', verbose=True):
        ...
        # Khởi tạo trạng thái: [giá, số dư, số cổ phiếu, MACD, RSI, CCI, ADX, VWAP, CORR_FX]
